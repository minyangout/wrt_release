...
update_package() {
    ...
    if [ -f "$mk_path" ]; then
        ...
        local PKG_VER=$(curl -sL "https://api.github.com/repos/$PKG_REPO/$branch" | jq -r '.[0] | .name // .tag_name')
        if [[ $branch == "tags" ]]; then
            local COMMIT_SHA=$(curl -sL "https://api.github.com/repos/$PKG_REPO/$branch" | jq -r '.[0].commit.sha' | cut -c1-7)
